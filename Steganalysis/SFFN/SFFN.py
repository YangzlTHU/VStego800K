...
class MODEL(nn.Module):
    def __init__(self, opt):
        ...
        self.params = list(self.cnn.parameters()) + list(self.cls.parameters())
        self.params_other = list(self.rnn1.parameters()) + list(self.cls1.parameters()) + list(
            self.rnn2.parameters()) + list(self.cls2.parameters())
...
def get_loaders(opt):
    ...
    val_cover_tensor = torch.tensor(val_cover)
    val_CNV_tensor = torch.tensor(val_CNV)
    val_pitch_tensor = torch.tensor(val_pitch)
    y_val_cover_tensor = torch.tensor(np.zeros(opt.num_val, ), dtype=torch.int64)
    y_val_positive_tensor = torch.tensor(np.ones(opt.num_val, ), dtype=torch.int64)

    # TensorDataset
    train_dataset = Data.TensorDataset(x_train_tensor, y_train_tensor)
    val_cover_dataset = Data.TensorDataset(val_cover_tensor, y_val_cover_tensor)
    val_CNV_dataset = Data.TensorDataset(val_CNV_tensor, y_val_positive_tensor)
    val_pitch_dataset = Data.TensorDataset(val_pitch_tensor, y_val_positive_tensor)
...
if __name__ == '__main__':
    ...
    for pretrain_path in ["rnn1", "rnn2"]:
        model_path = os.path.join(result_path_, pretrain_path, "model_best.pth.tar")
        if os.path.exists(model_path):
            print("=> loading checkpoint '{}'".format(model_path))
            checkpoint = torch.load(model_path)
            if pretrain_path == "rnn1":
                model.load_state_dict_pretrain1(checkpoint['model'])
            else:
                model.load_state_dict_pretrain2(checkpoint['model'])
        else:
